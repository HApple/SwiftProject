...
extension UIColor: JNNamespcaeWrappable {}

public extension JNTypeWrapperProtocol where JNWrappedType == UIColor {
    
    func toImage(size: CGSize = CGSize(width: 1, height: 1)) -> UIImage? {
        let rect = CGRect(x: 0, y: 0, width: size.width, height: size.height)
        UIGraphicsBeginImageContextWithOptions(size, false, 0)
        jnWrappedValue.setFill()
        UIRectFill(rect)
...
public extension UIColor {
    ...
    convenience init(hexString: String, alpha: Float = 1.0) {
        ...
        var initString = hexString
        if hexString.hasPrefix("#") {
            initString = hexString.replacingOccurrences(of: "#", with: "")
        }
        if hexString.hasPrefix("0x") {
            initString = hexString.replacingOccurrences(of: "0x", with: "")
        }
        
        let scanner = Scanner(string: initString)
        var hexValue: UInt64 = 0
        scanner.scanHexInt64(&hexValue)
        switch initString.count {
        case 3:
